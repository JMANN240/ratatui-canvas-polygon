...
impl Triangle {
    ...
    fn bounding_box_canvas(&self, bounds: (&[f64; 2], &[f64; 2])) -> Option<(f64, f64, f64, f64)> {
        ...
        if let (Some(x_min), Some(x_max), Some(y_min), Some(y_max)) = (maybe_x_min, maybe_x_max, maybe_y_min, maybe_y_max) {
    ...
    fn bounding_box_grid(&self, painter: &Painter) -> Option<(usize, usize, usize, usize)> {
        self.bounding_box_canvas(painter.bounds()).and_then(|bounds| {
